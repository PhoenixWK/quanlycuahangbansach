using BookStoreBLL;
using BookStoreDTO;
using System;
using System.Collections.Generic;
using System.Drawing;
using System.Linq;
using System.Windows.Forms;

namespace BookStoreGUI
{
    public partial class AuthorManagementPanel : UserControl
    {
        private readonly TacGiaBLL tacGiaBLL = new TacGiaBLL();
        private List<TacGiaDTO> authors = new List<TacGiaDTO>();
        private Panel authorCardsPanel;
        private TextBox searchTextBox;
        private Button addButton;

        public AuthorManagementPanel()
        {
            InitializeComponent();
        }

        private void InitializeComponent()
        {
            this.SuspendLayout();

            // Main panel setup
            this.BackColor = Color.FromArgb(245, 245, 245);
            this.Dock = DockStyle.Fill;
            this.Padding = new Padding(20);

            // Header panel
            Panel headerPanel = new Panel
            {
                Height = 60,
                Dock = DockStyle.Top,
                BackColor = Color.Transparent
            };

            // Title label
            Label titleLabel = new Label
            {
                Text = "QUẢN LÝ TÁC GIẢ",
                Font = new Font("Segoe UI", 16, FontStyle.Bold),
                ForeColor = Color.FromArgb(51, 51, 51),
                AutoSize = true,
                Location = new Point(0, 15)
            };

            // Search controls
            Panel searchPanel = new Panel
            {
                Height = 50,
                Dock = DockStyle.Top,
                BackColor = Color.Transparent,
                Padding = new Padding(0, 10, 0, 10)
            };

            searchTextBox = new TextBox
            {
                Width = 350,
                Height = 32,
                Location = new Point(0, 9),
                Font = new Font("Segoe UI", 11),
                BorderStyle = BorderStyle.FixedSingle,
                Text = "🔍 Tìm kiếm tác giả...",
                ForeColor = Color.FromArgb(150, 150, 150)
            };
            
            // Add placeholder functionality
            searchTextBox.Enter += (s, e) => {
                if (searchTextBox.Text == "ὐd Tìm kiếm tác giả...")
                {
                    searchTextBox.Text = "";
                    searchTextBox.ForeColor = Color.FromArgb(51, 51, 51);
                }
            };
            
            searchTextBox.Leave += (s, e) => {
                if (string.IsNullOrWhiteSpace(searchTextBox.Text))
                {
                    searchTextBox.Text = "ὐd Tìm kiếm tác giả...";
                    searchTextBox.ForeColor = Color.FromArgb(150, 150, 150);
                }
            };
            
            searchTextBox.TextChanged += SearchTextBox_TextChanged;

            Button searchButton = new Button
            {
                Text = "🔍 Search",
                Width = 120,
                Height = 32,
                Location = new Point(360, 9),
                BackColor = Color.FromArgb(0, 123, 255),
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 10, FontStyle.Bold),
                Cursor = Cursors.Hand
            };
            searchButton.FlatAppearance.BorderSize = 0;
            searchButton.FlatAppearance.MouseOverBackColor = Color.FromArgb(0, 103, 235);
            searchButton.Click += SearchButton_Click;

            Button refreshButton = new Button
            {
                Text = "🔄 Làm mới",
                Width = 120,
                Height = 32,
                Location = new Point(490, 9),
                BackColor = Color.FromArgb(108, 117, 125),
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 10),
                Cursor = Cursors.Hand
            };
            refreshButton.FlatAppearance.BorderSize = 0;
            refreshButton.FlatAppearance.MouseOverBackColor = Color.FromArgb(88, 97, 105);
            refreshButton.Click += RefreshButton_Click;

            addButton = new Button
            {
                Text = "➕ Thêm tác giả",
                Width = 160,
                Height = 32,
                Location = new Point(620, 9),
                BackColor = Color.FromArgb(40, 167, 69),
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 10, FontStyle.Bold),
                Cursor = Cursors.Hand
            };
            addButton.FlatAppearance.BorderSize = 0;
            addButton.FlatAppearance.MouseOverBackColor = Color.FromArgb(33, 136, 56);
            addButton.Click += AddButton_Click;

            // Cards container
            authorCardsPanel = new Panel
            {
                Dock = DockStyle.Fill,
                BackColor = Color.Transparent,
                AutoScroll = true,
                Padding = new Padding(0, 10, 0, 0)
            };

            // Add controls to panels
            searchPanel.Controls.AddRange(new Control[] { searchTextBox, searchButton, refreshButton, addButton });
            headerPanel.Controls.Add(titleLabel);

            // Add panels to main control
            this.Controls.Add(authorCardsPanel);
            this.Controls.Add(searchPanel);
            this.Controls.Add(headerPanel);

            this.ResumeLayout(false);

            // Load data
            LoadAuthors();
        }

        private void LoadAuthors()
        {
            try
            {
                authors = tacGiaBLL.LayDanhSachTacGia();
                DisplayAuthors(authors);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi khi tải danh sách tác giả: {ex.Message}", "Lỗi", 
                              MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void DisplayAuthors(List<TacGiaDTO> authorList)
        {
            authorCardsPanel.Controls.Clear();

            if (authorList.Count == 0)
            {
                Label noDataLabel = new Label
                {
                    Text = "Không có tác giả nàođược tìm thấy",
                    Font = new Font("Segoe UI", 12),
                    ForeColor = Color.FromArgb(108, 117, 125),
                    AutoSize = true,
                    Location = new Point(20, 50)
                };
                authorCardsPanel.Controls.Add(noDataLabel);
                return;
            }

            int cardWidth = 320; // Card width + margin
            int cardHeight = 135; // Card height + margin
            int cardsPerRow = Math.Max(1, authorCardsPanel.Width / cardWidth);
            
            int currentRow = 0;
            int currentCol = 0;
            
            foreach (var author in authorList)
            {
                Panel cardPanel = CreateAuthorCard(author);
                
                int xPosition = currentCol * cardWidth + 10;
                int yPosition = currentRow * cardHeight + 10;
                
                cardPanel.Location = new Point(xPosition, yPosition);
                authorCardsPanel.Controls.Add(cardPanel);
                
                currentCol++;
                if (currentCol >= cardsPerRow)
                {
                    currentCol = 0;
                    currentRow++;
                }
            }

            // Update scroll size
            if (authorCardsPanel.Controls.Count > 0)
            {
                int totalHeight = (currentRow + (currentCol > 0 ? 1 : 0)) * cardHeight + 20;
                authorCardsPanel.AutoScrollMinSize = new Size(0, totalHeight);
            }
        }

        private Panel CreateAuthorCard(TacGiaDTO author)
        {
            Panel cardPanel = new Panel
            {
                Width = 300,
                Height = 135,
                BackColor = Color.White,
                BorderStyle = BorderStyle.None,
                Margin = new Padding(10, 0, 10, 15)
            };

            // Add card shadow effect
            cardPanel.Paint += (s, e) =>
            {
                var rect = new Rectangle(0, 0, cardPanel.Width - 1, cardPanel.Height - 1);
                using (var pen = new Pen(Color.FromArgb(220, 220, 220), 1))
                {
                    e.Graphics.DrawRectangle(pen, rect);
                }
            };

            // Author ID Badge
            Label idBadge = new Label
            {
                Text = $"#{author.MaTG:D3}",
                Location = new Point(15, 15),
                Size = new Size(60, 25),
                Font = new Font("Segoe UI", 9, FontStyle.Bold),
                ForeColor = Color.White,
                BackColor = Color.FromArgb(0, 123, 255),
                TextAlign = ContentAlignment.MiddleCenter,
                BorderStyle = BorderStyle.None
            };

            // Author Name Label
            Label nameLabel = new Label
            {
                Text = author.TenTG,
                Location = new Point(85, 12),
                Size = new Size(200, 30),
                Font = new Font("Segoe UI", 12, FontStyle.Bold),
                ForeColor = Color.FromArgb(51, 51, 51),
                TextAlign = ContentAlignment.MiddleLeft
            };

            // Author info subtitle
            Label infoLabel = new Label
            {
                Text = "Tác giả",
                Location = new Point(85, 40),
                Size = new Size(150, 20),
                Font = new Font("Segoe UI", 9),
                ForeColor = Color.FromArgb(108, 117, 125),
                TextAlign = ContentAlignment.MiddleLeft
            };

            // Status indicator
            Label statusLabel = new Label
            {
                Text = "• Hoạt động",
                Location = new Point(85, 62),
                Size = new Size(100, 20),
                Font = new Font("Segoe UI", 8),
                ForeColor = Color.FromArgb(40, 167, 69),
                TextAlign = ContentAlignment.MiddleLeft
            };

            // Action Buttons Panel
            Panel buttonPanel = new Panel
            {
                Location = new Point(80, 80),
                Size = new Size(200, 40),
                BackColor = Color.Transparent
            };

            Button editButton = new Button
            {
                Text = "✏ Sửa",
                Location = new Point(0, 5),
                Size = new Size(75, 30),
                BackColor = Color.FromArgb(255, 193, 7),
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 8, FontStyle.Bold),
                Cursor = Cursors.Hand,
                Tag = author
            };
            editButton.FlatAppearance.BorderSize = 0;
            editButton.FlatAppearance.MouseOverBackColor = Color.FromArgb(230, 173, 7);
            editButton.Click += EditButton_Click;

            Button deleteButton = new Button
            {
                Text = "🗑 Delete",
                Location = new Point(85, 5),
                Size = new Size(75, 30),
                BackColor = Color.FromArgb(220, 53, 69),
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 8, FontStyle.Bold),
                Cursor = Cursors.Hand,
                Tag = author
            };
            deleteButton.FlatAppearance.BorderSize = 0;
            deleteButton.FlatAppearance.MouseOverBackColor = Color.FromArgb(200, 35, 51);
            deleteButton.Click += DeleteButton_Click;

            // Add all controls to button panel
            buttonPanel.Controls.AddRange(new Control[] { editButton, deleteButton });

            // Add all controls to card
            cardPanel.Controls.AddRange(new Control[] {
                idBadge, nameLabel, infoLabel, statusLabel, buttonPanel
            });

            // Add hover effect
            cardPanel.MouseEnter += (s, e) => {
                cardPanel.BackColor = Color.FromArgb(248, 249, 250);
                cardPanel.Cursor = Cursors.Default;
            };
            cardPanel.MouseLeave += (s, e) => cardPanel.BackColor = Color.White;

            return cardPanel;
        }

        private void SearchTextBox_TextChanged(object sender, EventArgs e)
        {
            if (searchTextBox.Text == "ὐd Tìm kiếm tác giả..." || string.IsNullOrWhiteSpace(searchTextBox.Text))
            {
                DisplayAuthors(authors);
            }
        }

        private void SearchButton_Click(object sender, EventArgs e)
        {
            string searchTerm = searchTextBox.Text.Trim();
            
            if (string.IsNullOrWhiteSpace(searchTerm) || searchTerm == "ὐd Tìm kiếm tác giả...")
            {
                DisplayAuthors(authors);
                return;
            }

            try
            {
                var searchResults = tacGiaBLL.TimKiemTacGia(searchTerm);
                DisplayAuthors(searchResults);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi khi tìm kiếm: {ex.Message}", "Lỗi", 
                              MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void RefreshButton_Click(object sender, EventArgs e)
        {
            searchTextBox.Text = "ὐd Tìm kiếm tác giả...";
            searchTextBox.ForeColor = Color.FromArgb(150, 150, 150);
            LoadAuthors();
        }

        private void AddButton_Click(object sender, EventArgs e)
        {
            var addForm = new AuthorForm();
            if (addForm.ShowDialog() == DialogResult.OK)
            {
                LoadAuthors();
            }
        }

        private void EditButton_Click(object sender, EventArgs e)
        {
            if (sender is Button button && button.Tag is TacGiaDTO author)
            {
                var editForm = new AuthorForm(author);
                if (editForm.ShowDialog() == DialogResult.OK)
                {
                    LoadAuthors();
                }
            }
        }

        private void DeleteButton_Click(object sender, EventArgs e)
        {
            if (sender is Button button && button.Tag is TacGiaDTO author)
            {
                var result = MessageBox.Show(
                    $"Bạn có chắc chắn muốn xóa tác giả '{author.TenTG}'?\n\nLưu ý: Không thể xóa tác giả đã có sách trong hệ thống!",
                    "Xác nhận xóa",
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question
                );

                if (result == DialogResult.Yes)
                {
                    try
                    {
                        if (tacGiaBLL.XoaTacGia(author.MaTG))
                        {
                            MessageBox.Show("Xóa tác giả thành công!", "Thông báo", 
                                          MessageBoxButtons.OK, MessageBoxIcon.Information);
                            LoadAuthors();
                        }
                        else
                        {
                            MessageBox.Show("Không thể xóa tác giả!", "Lỗi", 
                                          MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"Lỗi khi xóa tác giả: {ex.Message}", "Lỗi", 
                                      MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }

        public void RefreshData()
        {
            LoadAuthors();
        }
    }
}